...
class network(object):
    def __init__(self, list_of_num, batch_size):
        ...
        self.biases = [np.random.uniform(-0.5, 0.5,size = (1,y)) for y in list_of_num[1:]]
        self.weights = [np.random.uniform(-0.5, 0.5, size = (x,y)) for x,y in zip(list_of_num[:-1], list_of_num[1:])]
...
def train_lib_svm(x_train, y_train, x_test, y_test):
    #y_train, x_train = svm_read_problem("train_ass4_c.csv")
    #y_test, x_test = svm_read_problem("test_ass4_c.csv")
    #x_tr = pd.Series(x_train)
    #x_te = pd.Series(x_te)
    m = svm_train(y_train, x, '-c 1')
    res, acc, val = svm_predict(y_test ,x_te, m)
...
def main():
   ...
   # print(train_data)
    ...
    #train_lib_svm(x_train, y_train, x_test, y_test)
